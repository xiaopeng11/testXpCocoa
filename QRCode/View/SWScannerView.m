...
- (void)sw_addScannerLineAnimation {
    ...
    // 重置动画运行速度为1.0
    self.scannerLine.layer.speed = 2.0;
}
...
- (UIImageView *)scannerLine {
    if (!_scannerLine) {
        ...
        _scannerLine.image = [UIImage imageNamed:@"QRLine.png"];
    }
    return _scannerLine;
}
...
- (UILabel *)tipLab {
    if (!_tipLab) {
        _tipLab = [[UILabel alloc]initWithFrame:CGRectMake(0, Scanner_Y - 50, self.frame.size.width, 50)];
        _tipLab.textAlignment = NSTextAlignmentCenter;
        _tipLab.textColor = [UIColor whiteColor];
        _tipLab.font = [UIFont systemFontOfSize:14];
        _tipLab.text = @"将二维码放入框内";
    }
    return _tipLab;
}
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"ocr_flash-off.png"] forState:UIControlStateNormal];
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"ocr_flash-on.png"] forState:UIControlStateSelected];
    }
